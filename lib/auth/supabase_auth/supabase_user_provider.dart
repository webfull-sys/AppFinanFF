...
class AppFinanSupabaseUser extends BaseAuthUser {
  AppFinanSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> appFinanSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppFinanSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
